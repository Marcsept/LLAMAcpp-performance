            ...
            for k in range(len(fichiers_txt)):
                if(k>j):
                    with open(fichiers_txt[k], "r") as f:
                        content = f.read()
                        line_content = content.splitlines()
                        set3=[x.split("/")[0] for x in line_content]
                        set3_name=[x.split("/")[1] for x in line_content]
                        f.close()
                    combine3_set, combine3_set_name = combine(combine2_set, combine2_set_name, set3, set3_name)
                    #final_set, final_set_name = final_set + combine3_set, final_set_name + combine3_set_name 
                    for l in range(len(fichiers_txt)):
                        if(l>k):
                            with open(fichiers_txt[l], "r") as f:
                                content = f.read()
                                line_content = content.splitlines()
                                set4=[x.split("/")[0] for x in line_content]
                                set4_name=[x.split("/")[1] for x in line_content]
                                f.close()
                            combine4_set, combine4_set_name = combine(combine3_set, combine3_set_name, set4, set4_name)
                            final_set, final_set_name = final_set + combine4_set, final_set_name + combine4_set_name 
